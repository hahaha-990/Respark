...
$expired = db()->prepare("
    SELECT b.id, b.user_id, b.slot_id, ps.slot_code, b.class_start, b.class_end,
...
foreach ($expired->fetchAll() as $bk) {
    ...
    db()->prepare("INSERT INTO notifications (booking_id, user_id, type, lang, message_body) VALUES (?,?,?,?,?)")
       ->execute([$bk['id'], $bk['user_id'], 'auto_cancel', $lang, $lang==='ms'?$msgMs:$msgEn]);
    echo "[CRON] Auto-cancelled booking #{$bk['id']} for {$bk['user_id']}\n";
}

// ── 2. 15-min pre-class reminder ─────────────────────────────
$reminderTime = date('Y-m-d H:i:s', strtotime("+15 minutes", strtotime($now)));
$upcoming = db()->prepare("
    SELECT b.id, b.user_id, b.class_start, ps.slot_code, cz.name_ms, cz.name, u.lang_pref
    FROM bookings b
    JOIN parking_slots ps ON b.slot_id = ps.id
    JOIN campus_zones cz ON ps.zone_id = cz.id
    LEFT JOIN users u ON b.user_id = u.user_id
...
foreach ($upcoming->fetchAll() as $bk) {
    ...
    db()->prepare("INSERT INTO notifications (booking_id, user_id, type, lang, message_body) VALUES (?,?,?,?,?)")
       ->execute([$bk['id'], $bk['user_id'], 'reminder_15min', $lang, $lang==='ms'?$msgMs:$msgEn]);
    echo "[CRON] Sent 15-min reminder for booking #{$bk['id']}\n";
}

// ── 3. 10-min exit warning at class end ──────────────────────
$endingSoon = db()->prepare("
    SELECT b.id, b.user_id, b.class_end, ps.slot_code, u.lang_pref,
...
foreach ($endingSoon->fetchAll() as $bk) {
    ...
    db()->prepare("INSERT INTO notifications (booking_id, user_id, type, lang, message_body) VALUES (?,?,?,?,?)")
       ->execute([$bk['id'], $bk['user_id'], 'expiry_10min', $lang, $lang==='ms'?$msgMs:$msgEn]);
